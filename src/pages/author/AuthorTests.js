...
const AuthorTests = () => {
  ...
  const fetchTests = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/tests/my-tests`);
      ...
      for (const test of testsData) {
        try {
          ...
          modulesData[test.id] = Array.isArray(modulesRes.data) ? modulesRes.data : [];
        } catch (error) {
          modulesData[test.id] = [];
        }
      }
            ...
            <div key={test.id} className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 text-white">
                <div className="flex justify-between items-start">
                  ...
                  <button onClick={() => deleteTest(test.id)} className="bg-red-600 hover:bg-red-700 px-3 py-1 rounded-lg text-sm flex items-center">
              ...
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <Link to={`/author/create-module/${test.id}`} className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700 flex items-center">
                      ...
                      <div key={module.id} className="border rounded-lg p-4 hover:bg-gray-50">
                        <div className="flex justify-between items-start">
                          ...
                          <div className="flex space-x-2 ml-4">
                            <button onClick={() => window.open(`/modules/${test.id}`, '_blank')} className="text-indigo-600 hover:text-indigo-700">
