        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
            ...
            <tbody>
              {attempts.map((attempt) => (
                <tr key={attempt.attemptId} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-4 px-6">
                    <Link to={`/result/${attempt.attemptId}`} className="flex items-center text-indigo-600">
          ...
          {attempts.length === 0 && (
            <div className="text-center py-12">
              <p className="text-gray-500">No attempts yet. Start your first test!</p>
              <Link to="/tests" className="inline-block mt-4 px-6 py-2 bg-indigo-600 text-white rounded-lg">
                Browse Tests
              </Link>
            </div>
          )}
