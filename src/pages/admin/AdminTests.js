...
const AdminTests = () => {
  ...
  const publishTest = async (testId) => {
    try {
      await axios.put(`${API_URL}/api/admin/tests/${testId}/publish`);
              ...
              {tests.map((test) => (
                <tr key={test.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-4 px-6">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${test.published ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                  ...
                  <td className="py-4 px-6">
                    <div className="flex space-x-2">
                      {!test.published && (
                        <button onClick={() => publishTest(test.id)} className="px-3 py-1 bg-green-600 text-white rounded-lg text-sm">
                          <CheckCircle className="inline h-4 w-4 mr-1" /> Publish
                        </button>
                      )}
                      <button onClick={() => deleteTest(test.id)} className="px-3 py-1 bg-red-600 text-white rounded-lg text-sm">
